<script lang="ts">
  ...
  const navItems = [
    ...
    { href: '/admin/currencies', label: 'Devises', icon: 'M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z' },
  ...
  ">
    ...
    <div class="px-3 py-4 border-t border-gray-200 dark:border-gray-800 space-y-2">
      ...
      <div class="pt-2">
        <a
          href="/logout"
          class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 transition-all duration-150"
        >
          <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          Déconnexion
          {#if data.user}
            <span class="ml-auto text-[10px] opacity-60 font-mono uppercase truncate max-w-[80px]">{data.user.username}</span>
          {/if}
        </a>
      </div>
    </div>
